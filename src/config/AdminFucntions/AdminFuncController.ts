...
class AdminFunctionController {
    ...
    async workReportController(req: Request, res:Response) {
        try {
            const userId = req.query.id;
            console.log('userId: ', userId);
            const datafromDB = await AdminFunctionServices.workReportServices(userId);
            console.log('Time in controller:',datafromDB.timeValues);
            res.status(200).json({message: 'User Details Found!', datafromDB})
        } catch (error) {
            console.log('error: ', error);
            res.status(500).json({ error: 'Internal Server Error' });
        }
